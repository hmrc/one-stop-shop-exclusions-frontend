...
import models.registration.{ContactDetails, UkAddress, VatDetails}
...
class AmendRegistrationRequestSpec extends SpecBase with Matchers {

  "AmendRegistrationRequest" - {


    "must deserialise/serialise to and from AmendRegistrationRequest" - {
      ...
      "when all optional values are present" in {
        ...
        val expectedResult = AmendRegistrationRequest(
          vrn = vrn,
          registeredCompanyName = "foo",
        ...
        json.validate[AmendRegistrationRequest] mustBe JsSuccess(expectedResult)
      }
    }
